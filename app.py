...
with col_pie:
    ...
    fig = px.pie(
        df_device, names="device", values="sessions",
    ...
    st.caption("Unique sessions broken down by device category across the full Nov 2020 – Jan 2021 period.")

st.divider()

# ── Row 1: Engagement Rate | Conversion Rate ──────────────────────────────────
...
with col_l:
    ...
    fig.update_layout(
        yaxis_range=[0, 100], plot_bgcolor="rgba(0,0,0,0)",
...
with col_r:
    ...
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font_color="#ccc", margin=dict(t=10, b=0),
    )
    st.plotly_chart(fig, use_container_width=True)
    st.caption("% of sessions that resulted in at least one purchase event.")


# ── Row 2: Revenue by Channel | AOV Trend ─────────────────────────────────────
...
with col_r:
    ...
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
        font_color="#ccc", yaxis_title="AOV (USD)",
        xaxis_title="", margin=dict(t=10, b=0),
    )
    st.plotly_chart(fig, use_container_width=True)
    st.caption("Total revenue divided by number of transactions — shows how purchase value shifts during holiday season.")


# ── Row 3: Conversion Funnel | Revenue Trend ──────────────────────────────────

col_l, col_r = st.columns(2)

with col_l:
    st.markdown('<p class="section-title">Cart Abandonment — Conversion Funnel</p>', unsafe_allow_html=True)
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.caption(f"Cart abandonment rate: **{abandonment_rate}%** of sessions that added to cart did not complete a purchase.")
...
with col_r:
    ...
    fig.add_trace(go.Bar(
        ...
        name="Revenue (USD)", marker_color=COLORS["warning"],
        yaxis="y1",
    ))
    fig.add_trace(go.Scatter(
...
# ── Implementation 1: Sessions by Country (Choropleth) ────────────────────────
...
fig.update_layout(
    ...
    font_color="#ccc",
    coloraxis_colorbar=dict(title=dict(text="Sessions", font=dict(color="#ccc")), tickfont=dict(color="#ccc")),
    margin=dict(t=10, b=10, l=0, r=0),
    height=450,
)
st.plotly_chart(fig, use_container_width=True)
st.caption("Session volume by country across Nov 2020 – Jan 2021. Darker blue = more sessions. Identifies which markets drive the most traffic.")
...
st.plotly_chart(fig, use_container_width=True)
st.caption("Daily revenue (solid, left axis) alongside daily sessions (dotted, right axis). Reveals whether traffic spikes — e.g. Black Friday, Cyber Monday — translate into proportional revenue gains.")
...
# ── Implementation 5: Page Flow Sankey ────────────────────────────────────────
...
# Identify top 8 pages by total appearance in the transition data
